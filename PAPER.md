...
Es un framework de codificación formado a partir de la combinación de distintas CNN. Deepcoder usa la cuantización escalar y la codificación Huffman para obtener un mapa de las activaciones encontradas en la imagen por las CNN (fMap), para usarlo en la intrapredicción del proceso de compresión. Luego los residuales de la intra e inter predicción son codificados en fMaps de manera similar que con la intrapredicción, pero con diferentes parámetros. Como resultado, se obtuvo un grado de compresión comparable con el del codec H.264.
...
1. La estimación de movimiento se da calculando el flujo óptico de los fotogramas, usando vectores de movimiento denotados por $\hat v_t$. Estos son calculados a traves de una serie de redes convolucionales para reducir su tamaño, dando como resultado la cuantización del flujo óptico $\hat m_t$
2. Una red neuronal se especializa en predecir los fotogramas usando el flujo óptico obtenido en el paso 1, obteniendo los fotogramas predecidos $\bar x_t$
3. Se usa una transformación no linear que lleve los residuos (obtenidos de sustraer los fotogramas predecidos de los originales) $r_t$ hacia $y_t$. Luego $y_t$ es llevado a  $\hat y_t$
4. Se usa  $\hat y_t$ como entrada a una red neuronal que obtiene el valor del residual reconstruido, $\hat r_t$
5. Se usa una CNN para obtener la distribución de probabilidad de cada símbolo codificado en $\hat m_t$ y  $\hat y_t$.
